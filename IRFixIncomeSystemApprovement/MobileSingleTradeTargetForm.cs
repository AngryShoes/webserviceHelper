...
{
    public partial class MobileSingleTradeTargetForm : Form
    {
        ...
        public MobileSingleTradeTargetForm()
        {
            InitializeComponent();
        }

        private void btn_mobileSingleTradeTarget_Click(object sender, EventArgs e)
        {
            //获取报文
            //string xmlString = HttpWebRequestProcessor.PostWebService(_url, "MobileSingleTradeTarget", GetSingleTradeTargetMethod());
 ...
 <RE>
  ...
  <TARGETINDEX NAME=""交易费用(组合)"" VALUE=""412.50"" />
            ...
            //处理报文
            var singleTradeTargetDictionary = MobileSingleTradeTargetXmlProcess.ProcessMobileSingleTradeTargetXml(xmlString, ref successInfo);
            if (successInfo == "查询成功")
            {
                MessageBox.Show(successInfo);
            }
            //显示查询数据
            foreach (var keyValuePair in singleTradeTargetDictionary)
            {
                lbx_singleTradeTarget.Items.Add(keyValuePair.Key + " " + keyValuePair.Value);
            }
        }

        private void MobileSingleTradeTargetForm_Load(object sender, EventArgs e)
        {
        ...
        private Hashtable GetSingleTradeTargetMethod()
        {
